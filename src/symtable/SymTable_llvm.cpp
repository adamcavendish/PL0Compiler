...
SymTable_llvm::SymTable_llvm() :
    m_pfunction_closure_map(std::make_shared<
...
bool
SymTable_llvm::createVariable(const std::string & name, llvm::Value * inst) {
...
bool
SymTable_llvm::setVariable(const std::string & name, llvm::Value * inst) {
    if(m_sym_arr.empty())
        return false;
